                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            @can('business-category-add-btn')
                                <a class="btn btn-info" href="{{ route('notification.create') }}">Add</a>
                            @endcan
                                ...
                                <table id="example" class="display" style="min-width: 845px;color:black">
                                    ...
                                    <tbody>
                                        @foreach ($datas as $key => $value)
                                            <tr>
                                                <td>{{ ++$key }}</td>
                                                <td>{{ $value->user->name ?? 'ALL' }}</td>
                                                <td><img class="w-50" src="{{asset('/')}}{{$value->image}}"> </td>
                                                <td>{{ $value->remarks }}</td>
                                                <td>
                                                    @if ($value->status == 1)
                                                        <span
                                                ...
                                                <td class="text-center align-middle">
                                                    <div class="d-flex justify-content-center align-items-center">
                                                        ...
                                                        <a href="{{ route('notification.edit', $value->id) }}"

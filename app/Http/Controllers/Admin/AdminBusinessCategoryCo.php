...
class AdminBusinessCategoryCo extends Controller
{
    ...
    {
        $datas = TBusinessCategory::with('keywords')->latest()->get();
    ...
    public function store(TBusinessCategoryRequest $request)
    {
        ...
        try{
            ...
            $selectedKeywords = array_filter(array_map('trim', explode(',', $request->keyword)));

            foreach($selectedKeywords as $keyword) {
                TBusinessCategoryKeyword::create([
                    'category_id'       => $data->id,
                    'name'              => $keyword,
                    'created_by'       => Auth::id(),
                ]);
    ...
    {
        $data = TBusinessCategory::with('keywords')->find($id);
    ...
    public function update(TBusinessCategoryRequest $request, string $id)
    {
        ...
        try{
            ...
            // insert into t_business_category_keywords table
            TBusinessCategoryKeyword::where('category_id', $data->id)->delete();
            $selectedKeywords = array_filter(array_map('trim', explode(',', $request->keyword)));
            foreach($selectedKeywords as $keyword) {
                TBusinessCategoryKeyword::create([
                    'category_id'       => $data->id,
    ...
    public function destroy(string $id)
    {
        try{
            ...
            return redirect()->route('business-category.index')->with('warning','Data Deleted Successfully');
        } catch (\Exception $e) {
